...
export default {
  ...
  created() {
    this.$axios
      .$get("/api/patientusers/" + this.username + "/email/receive/" + this.code) 
      .then((mail) => {
          console.log(mail)
        this.mail = mail;
      });
  ...
  methods: {
      responde(){
          this.$router.push('/patients/profhealthcare/sendEmailProfHealthcare')
      },

      deleteMsg(){
          this.$axios.delete('api/patientusers/' + this.username + '/email/delete/' + this.code)
            .then(()=>{
