       ...
       <b-container>
        ...
        <b-table v-if="mails.length" striped over :items="mails" :fields="fields">
          <template v-slot:cell(actions)="row">
            <b-button
                    variant="info"
                    :to="`/patients/mails/${row.item.code}`"
                >
...
<script>
  export default {
    ...
    created(){
        this.$axios.get('api/patientusers/' + this.username + '/email/receive')
        .then((mail)=>{
    ...
    methods: {
        removeMessage(code)
        {
            this.$axios.delete('api/patientusers/' + this.username + '/email/delete/' + code)
            .then(()=>{
